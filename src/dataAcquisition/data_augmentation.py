    ...
    for img_idx, (img, filename, class_name) in enumerate(images):
        ...
        while created < num_aug_per_image:
            try:
                ...
                # Skip on error and try again
                continue

    if show_progress:
        print()  # New line after progress
        print(f"Data augmentatie voltooid! Totaal: {len(augmented)} afbeeldingen ({total_images} origineel + {len(augmented) - total_images} gegenereerd)")
